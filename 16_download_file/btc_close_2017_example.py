...
req = response.read()
# 将数据写入文件
with open(filepath + '/btc_close_2017_urllib.json', 'wb') as f:  # 3
    f.write(req)
# 加载json格式
...
try:
    ...
    # 将数据写入文件
    with open(filepath + '/btc_close_2017_request.json', 'w') as f:
        f.write(req.text)  # 2
...
line_chart.render_to_file(filepath + '/收盘价对数变换折线图（¥）.svg')
line_chart
...
try:
    ...
    line_chart_weekday.render_to_file(filepath + '/收盘价星期均值（¥）.svg')
    line_chart_weekday
